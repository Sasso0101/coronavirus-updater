...
if somethingChanged(nationalData) or force:
    ...
    if not notificationSentToday():
        ...
        telegramMessage = '<b>Bollettino coronavirus - {}</b>\r\n🟧 {:n} contagi oggi ({:n} attualmente positivi)\r\n🟥 {:n} decessi ({:n} decessi totali)\r\n🟩 {:n} guariti oggi ({:n} guariti totali)\r\n⬜️ {:n} tamponi effettuati oggi ({:n} tamponi totali)\r\n\r\n{:n} in terapia intensiva || {:n} ospedalizzati\r\n\r\n{:n} vaccini somministati (su {:n} dosi disponibili)\r\nVedi i casi nella tua regione su <a href=\"https://coronaviruslive.it/?utm_source=telegram&utm_medium=message&utm_campaign=telegram_message\">coronaviruslive.it</a>'.format(now.strftime("%d/%m/%Y"), data['today']['newActiveCases'], data['today']['activeCases'], data['today']['newDeaths'], data['today']['deaths'], data['today']['newRecovered'], data['today']['recovered'], data['today']['newTests'], data['today']['tests'], data['today']['intensiveCare'], data['today']['hospitalized'], vaccineData['vaccinated'], vaccineData['availableVaccines'])
